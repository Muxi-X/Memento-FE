...
export default function HomeCard(props: Props) {
  return (
    <View style={styles.card}>
      {props.cover ? (
        <Image source={{ uri: props.cover?.variants.square_small.url }} style={styles.Imagedata} />
      ) : (
        <CoverImage></CoverImage>
      )}

      <Text style={{ fontSize: 18, color: '#3D3D3D', marginLeft: 12 }}>{props.title}</Text>
      <View
        style={{
          flexDirection: 'row',
          alignItems: 'center',
      ...
      >
        ...
        <Pressable
          ...
          onPress={() => {
            router.navigate({
              pathname: '/customPage',
              params: { keyword: props.title, keyword_id: props.keyword_id },
            });
          }}
        >
